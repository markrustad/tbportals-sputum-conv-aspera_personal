...
tab1_vars <- df_pos_init %>%
  select(age_of_onset, bmi, gender, country, case_definition, registration_date, 
         type_of_resistance_2, lung_localization, x_ray_exists, ct_exists,
         genomic_data_exists) %>%
  distinct() 

#Create labels for table
labels(tab1_vars)  <- c(type_of_resistance_2 = 'Type of Resistance',
                        registration_date = "Registration Date",
                        age_of_onset = "Age of onset (yrs)",
                        bmi = "BMI",
                        gender = "Gender",
                        country = "Country",
                        case_definition = "Case definition",
                        lung_localization = "Lung localization",
                        x_ray_exists = "Radiographs in portal",
                        ct_exists = "CT in portal",
                        genomic_data_exists = "Genomic data in portal")
...
tab1 <- tableby(type_of_resistance_2 ~ registration_date + age_of_onset + bmi +
                  gender + country + case_definition + lung_localization +
                  x_ray_exists + ct_exists + genomic_data_exists,
                data=tab1_vars,
                control = mycontrols)
